package com.king.dactylology.LoginModule.Utils.UtilsItems;


import com.tencentcloudapi.common.Credential;
import com.tencentcloudapi.common.exception.TencentCloudSDKException;
import com.tencentcloudapi.sms.v20190711.SmsClient;
import com.tencentcloudapi.sms.v20190711.models.SendSmsRequest;
import com.tencentcloudapi.sms.v20190711.models.SendSmsResponse;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Component;

import java.util.*;


/**
 * 短信验证码相关的操作
 */
@Component
public class Check {
    //存放验证码本身
    private static Map<String,String> codeMap;

    //存放验证码过期时间
    private static Map<String,Date> dateMap;

    //验证码位数
    private static final int n = 4;

    //验证码有效期，单位为分钟
    private static final int deadmin = 10;

    //初始化
    static{
        codeMap = new Hashtable<>();
        dateMap = new Hashtable<>();
    }

    //操作date对象
    @Autowired
    @Qualifier("dateOpeartor")
    DateOpeartor dateOpeartor;

    /**
     * 发送验证码
     * @param phoneNumber 手机号
     */
     void sendMessage(String phoneNumber){
        //清除垃圾
        this.removeRubbish();
        //发送code
        String code = this.createCode(n);
        if(codeMap.containsKey(phoneNumber)){
            //存在
            codeMap.replace(phoneNumber,code);
            dateMap.replace(phoneNumber,dateOpeartor.addDateMin(new Date(),deadmin));
        }else{
            //不存在已发送的验证码
            codeMap.put(phoneNumber,code);
            dateMap.put(phoneNumber,dateOpeartor.addDateMin(new Date(),deadmin));
        }
        //发送验证码
        this.doSendMessageByTencent(phoneNumber,code);
    }

    /**
     * 验证code
     * @param phoneNumber 手机号
     * @param code 用户输入的验证码
     * @return
     */
    boolean checkMessage(String phoneNumber,String code){
        if(codeMap.containsKey(phoneNumber)){
            String true_code = codeMap.get(phoneNumber);
            Date deadTime = dateMap.get(phoneNumber);
            if(true_code.equals(code)&&deadTime.after(new Date())){
                //check后删除code
                dateMap.remove(phoneNumber);
                codeMap.remove(phoneNumber);
                return true;
            }
            dateMap.remove(phoneNumber);
            codeMap.remove(phoneNumber);
        }
        return false;
    }

    /**
     * 调用腾讯SDK发送验证码短信
     * @param phoneNum
     * @param code
     */
    private void doSendMessageByTencent(String phoneNum,String code){
        try {
            //CAM 密钥查询：https://console.cloud.tencent.com/cam/capi
            String secretId = "AKIDiccBVcp6SxxkFCHiHcgh3AWcpoWx9H8O";
            String secretKey = "83vWeZHisJYYKp7C1uYyA6rdAPOC3ez4";
            //短信应用 ID: 在 [短信控制台] 添加应用后生成的实际 SDKAppID，例如1400006666
            String appid = "1400494731";
            /* 短信签名内容: 使用 UTF-8 编码，必须填写已审核通过的签名，可登录 [短信控制台] 查看签名信息 */
            String sign = "LWZ的小站";
            /* 模板 ID: 必须填写已审核通过的模板 ID，可登录 [短信控制台] 查看模板 ID */
            String templateID = "889195";

            /* 必要步骤：
             * 实例化一个认证对象，入参需要传入腾讯云账户密钥对 secretId 和 secretKey
             * 本示例采用从环境变量读取的方式，需要预先在环境变量中设置这两个值
             * 您也可以直接在代码中写入密钥对，但需谨防泄露，不要将代码复制、上传或者分享给他人
             * CAM 密钥查询：https://console.cloud.tencent.com/cam/capi
             */
            Credential cred = new Credential(secretId, secretKey);
            // 实例化一个 http 选项，可选，无特殊需求时可以跳过

            /* 实例化 SMS 的 client 对象
             * 第二个参数是地域信息，可以直接填写字符串 ap-guangzhou，或者引用预设的常量 */
            SmsClient client = new SmsClient(cred, "");

            /* 实例化一个请求对象，根据调用的接口和实际情况，可以进一步设置请求参数
             * 您可以直接查询 SDK 源码确定接口有哪些属性可以设置
             * 属性可能是基本类型，也可能引用了另一个数据结构
             * 推荐使用 IDE 进行开发，可以方便地跳转查阅各个接口和数据结构的文档说明 */
            SendSmsRequest req = new SendSmsRequest();
            /* 填充请求参数，这里 request 对象的成员变量即对应接口的入参
             * 您可以通过官网接口文档或跳转到 request 对象的定义处查看请求参数的定义
             * 基本类型的设置:
             * 帮助链接：
             * 短信控制台：https://console.cloud.tencent.com/smsv2
             * sms helper：https://cloud.tencent.com/document/product/382/3773 */

            ;
            req.setSmsSdkAppid(appid);

            req.setSign(sign);
            /* 国际/港澳台短信 senderid: 国内短信填空，默认未开通，如需开通请联系 [sms helper] */
            String senderid = "";
            req.setSenderId(senderid);
            /* 用户的 session 内容: 可以携带用户侧 ID 等上下文信息，server 会原样返回 */
            String session = "";
            req.setSessionContext(session);
            /* 短信码号扩展号: 默认未开通，如需开通请联系 [sms helper] */
            String extendcode = "";
            req.setExtendCode(extendcode);
            req.setTemplateID(templateID);
            /* 下发手机号码，采用 e.164 标准，+[国家或地区码][手机号]
             * 例如+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号*/
            String[] phoneNumbers = {"+86"+phoneNum};
            req.setPhoneNumberSet(phoneNumbers);
            /* 模板参数: 若无模板参数，则设置为空*/
            String[] templateParams = {code};
            req.setTemplateParamSet(templateParams);
            /* 通过 client 对象调用 SendSms 方法发起请求。注意请求方法名与请求对象是对应的
             * 返回的 res 是一个 SendSmsResponse 类的实例，与请求对象对应 */
            SendSmsResponse res = client.SendSms(req);
            // 输出 JSON 格式的字符串回包
            System.out.println(SendSmsResponse.toJsonString(res));
            // 可以取出单个值，您可以通过官网接口文档或跳转到 response 对象的定义处查看返回字段的定义
            System.out.println(res.getRequestId());
        } catch (TencentCloudSDKException e) {
            e.printStackTrace();
        }
    }

    /**
     * 清除过期的验证码
     */
    private void removeRubbish(){
        try {
            for (String key : codeMap.keySet()) {
                if(dateMap.get(key).before(new Date())){
                    //清除，已过期
                    dateMap.remove(key);
                    codeMap.remove(key);
                }
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    }

    /**
     * 生成n位随机数的验证码
     * @param n
     * @return
     */
    private String createCode(int n){
        Random random = new Random();
        StringBuilder code = new StringBuilder();
        for(int i =0;i<n;i++){
            code.append(random.nextInt(10));
        }
        return code.toString();
    }

}
